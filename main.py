...
def compute_depth(event, depth_map, baseline, focal_length, scan_speed, start_time):
  ...
  if disparity != 0:
    ...
    depth = 10000     #data is captured for a horizontal line hence x
  #print(y, x, depth)
  depth_map[int(x)][int(y)].append(depth)

def compute_final_depth_map(depth_map):
  depth_map_new = [[sum(depth_map[i][j])/ len(depth_map[i][j]) if len(depth_map[i][j])!= 0 else 10000 for j in range(346)] for i in range(260)]
...
def main():
  ...
  baseline = 0.015
  scan_speed = 62.5 #pixels per second coverge
